...
module.exports = welcome = async (vinzbot, anu) => {
		...
		try {
			    ...
			    console.log(anu)
                try {
                pp_user = await vinzbot.getProfilePicture(mem)
                } catch (e) {
                pp_user = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
            }
                try {
                pp_grup = await vinzbot.getProfilePicture(anu.jid)
                } catch (e) {
                pp_grup = 'https://i.postimg.cc/SN54m6LW/SAVE-20210728-133334.jpg'
            }
            if (anu.action == 'add' && mem.includes(vinzbot.user.jid)) {
            vinzbot.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Ketik ${prefix}menu', 'conversation')
            }
             if (anu.action == 'add' && !mem.includes(vinzbot.user.jid)) {
             if (!welkom.includes(anu.jid)) return
                mdata = await vinzbot.groupMetadata(anu.jid)
                memeg = mdata.participants.length
            	num = anu.participants[0]
                let v = vinzbot.contacts[num] || { notify: num.replace(/@.+/, '') }
                ...
                imageMsg = (await vinzbot.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${teks}`, footerText: 'JANGAN BUAT ONAR BRO', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
                prep = await vinzbot.prepareMessageFromContent(mdata.id,{buttonsMessage},{})
                vinzbot.relayWAMessage(prep)
}
            if (anu.action == 'remove' && !mem.includes(vinzbot.user.jid)) {
            if (!welkom.includes(anu.jid)) return
                mdata = await vinzbot.groupMetadata(anu.jid)
            	num = anu.participants[0]
                let w = vinzbot.contacts[num] || { notify: num.replace(/@.+/, '') }
